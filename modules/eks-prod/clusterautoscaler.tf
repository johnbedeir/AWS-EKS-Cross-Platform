...
locals {
  ...
  autoscaler_cluster_name              = aws_eks_cluster.proc_eks.name
}
...
resource "aws_iam_openid_connect_provider" "eks" {
  ...
  url             = aws_eks_cluster.proc_eks.identity[0].oidc[0].issuer
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_eks_cluster.proc_eks.identity[0].oidc[0].issuer, "https://", "")}:aud" = "sts.amazonaws.com",
            "${replace(aws_eks_cluster.proc_eks.identity[0].oidc[0].issuer, "https://", "")}:sub" = "system:serviceaccount:kube-system:cluster-autoscaler"
          }
        }
      }
    ]
